...
class SplashScreen : AppCompatActivity(), Animation.AnimationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imgview = findViewById<ImageView>(R.id.imageanimation)
    ...
    override fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        if (hasFocus) {
            guniAnimation.start()
            findViewById<ImageView>(R.id.imageanimation).startAnimation(tweenAnimation)
        } else {
            guniAnimation.stop()
        }
    }
